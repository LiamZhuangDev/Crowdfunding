...
export default function CreateCampaignModal({ onClose, onSuccess }: CreateCampaignModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Creating campaign with:', { name, description, goal, duration: parseInt(duration) });
      setIsCreating(true);
      await createCampaign(name.trim(), description.trim(), goal, parseInt(duration));
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                maxLength={500}
                rows={4}
                className="w-full px-4 py-3 border-2 border-gray-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-slate-800 dark:text-white transition-all shadow-sm hover:shadow-md resize-none"
                placeholder="介绍项目背景、资金用途和希望达成的目标"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400">
                描述将会写入链上，便于支持者了解项目
              </p>
